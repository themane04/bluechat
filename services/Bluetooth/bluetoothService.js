...
class BluetoothService {
  constructor() {
    ...
    this.receiveMessageCharacteristicUuid =
      'cb16b8ba-3e58-414a-a74e-5de844436ef2';
    ...
    const nativeModule =
      NativeModules.CustomBluetooth || NativeModules.BluetoothClient;
    ...
    if (sdk >= 31) {
      ...
      if (!has) {
        ...
        if (res !== PermissionsAndroid.RESULTS.GRANTED)
          throw new Error('Missing ACCESS_FINE_LOCATION');
      }
    }
  }
  ...
  async StartAdvertising() {
    try {
      ...
      const bt = NativeModules.CustomBluetooth;
      if (!bt) {
        console.error(
          'Native module CustomBluetooth is not available. NativeModules keys:',
          Object.keys(NativeModules),
        );
        throw new Error(
          'Native module CustomBluetooth not found — rebuild the app or register the package in MainApplication.java',
        );
      }

      await bt.addService(this.serviceUuid, true);
      await bt.addCharacteristicToService(
        this.serviceUuid,
      ...
      bt.onCharacteristicWriteRequest(
        this.serviceUuid,
        this.receiveMessageCharacteristicUuid,
        async (deviceId, value) => {
          try {
            ...
            var msg2 = msg.split(':');
            var data = {
              device: this.connectedDevices.find(x => x.username === msg2[0]),
              message: msg2[1],
            };
      ...
      await bt.setCharacteristicValue(
        this.serviceUuid,
        this.usernameCharacteristicUuid,
        b64,
      );
  ...
  setReadResponse(serviceUUID, charUUID, str) {
    ...
    try {
      ...
      if (
        CustomBluetooth &&
        typeof CustomBluetooth.setCharacteristicValue === 'function'
      ) {
  ...
  async safeReadCharacteristicValue(device) {
    ...
    return this._enqueueRead(async () => {
      ...
      try {
        // Quick pre-check
        const alreadyConnected = await this.manager
          .isDeviceConnected(device.id)
          .catch(() => false);
        if (!alreadyConnected) {
          device = await this.manager.connectToDevice(device.id, {
            timeout: 10000,
          });
          connectedHere = true;
        }
        ...
        try {
          ...
          /* ignore, sometimes already discovered */
        }
        ...
        console.error(
          '[BluetoothService] safeReadCharacteristicValue failed',
          device.id,
          e && e.message,
        );
        return '';
      ...
      async (error, device) => {
        ...
        try {
          if (!filter || (device.name && device.name.includes(filter))) {
            var find = this.connectedDevices.find(
              d => d.rawScanRecord === device.rawScanRecord,
            );
            if (find) {
              find.id = device.id;
              return;
            ...
            DeviceEventEmitter.emit('bleDeviceFound', {
              device: this.connectedDevices.find(
                d => d.rawScanRecord === device.rawScanRecord,
              ),
            });
          }
    ...
    try {
      ...
      try {
        if (!(await this.manager.isDeviceConnected(device.id))) {
          connectedDev = await this.manager.connectToDevice(device.id, {
            timeout: 8000,
          });
